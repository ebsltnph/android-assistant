...
class UpdateBufferTool(
    ...
    override val description =
        "update_buffer(items?, archive_ids?, delete_ids?)：「进行中的事」缓冲区维护工具——" +
            ...
            "**收录标准（先判断，再动手）**：只记「状态」，不记「见闻」。判断标准就一句——" +
            "**这条以后还会影响你的行为吗？不会就别记。**\n" +
            "- 该记：在办事项做到哪一步、下一步是什么、卡在哪、在等谁或等什么结果；" +
            "需要盯一段时间的状况（身体不适、等待某个结果、某件事的截止日）。\n" +
            "- **不该记**：一次性的见闻与细节（去过哪、看到什么设备、跟谁聊了什么、看了哪一集、" +
            "某个清单/名录/参数表本身）、闲聊与情绪、已经彻底结束且不会再提的小事。" +
            "这类内容如果值得留档，它的位置是日记而不是这里。\n" +
            "- 同一件事只留一条，写**当前**状态：新进展覆盖旧进展，不要攒成流水账。\n" +
            "- 宁少勿多：拿不准要不要记的，就别记。\n" +
            "参数：\n" +
            "- items：要新增或改写的条目数组，每项 {\"id\"?:编号, \"kind\"?:\"progress|watch\", " +
            "\"title\":事项名（2-8 个字）, \"body\":浓缩正文, \"diary_ids\"?:[日记编号]}。" +
            ...
            "- body：**一两句话、100 字以内**，只写结论与状态（如「已完成：…；待办：…」）。" +
            "**不要把过程细节抄进来**——有对应日记时用 diary_ids 挂上编号即可（细节留在日记里，" +
            "你以后需要时再 read_diary 去读）。专有名词与关键数值（题目、参数、结论性数字）保留原样，" +
            "但只保留**以后还用得上的**那些。\n" +
            ...
            "\"body\":\"已完成：报名交费；待办：每天背 50 个单词\"}],\"archive_ids\":[3]}"
