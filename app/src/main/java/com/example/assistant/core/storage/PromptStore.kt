...
- 相对时间（明天九点、半小时后等）换算成 set_reminder 需要的结构化参数时，以最后一条用户消息开头的时间戳为基准；具体日期计算交给程序
...
- 不要编造素材里没有的内容"""
        ...
        /**
         * 识屏三个快捷动作的提示词（2026-09-11 起）：识图已并入聊天通道，
         * 按钮 = "该提示词 + 图片"作为一轮普通对话发送（不再有独立的识屏提示词与独立通道）。
         */
        SCREEN_ACTION_EXTRACT(
            "识屏·提取文字",
            "识屏面板「提取文字」按钮发送的提示词（随图片一起作为一轮对话）",
            stringPreferencesKey("screen_action_extract_prompt"),
            """请提取这张图片上的全部文字，按原有顺序和布局整理输出，不要遗漏、不要编造。
只输出提取结果，不要添加解释或客套话。"""
        ),

        SCREEN_ACTION_TRANSLATE(
            "识屏·翻译",
            "识屏面板「翻译」按钮发送的提示词（随图片一起作为一轮对话）",
            stringPreferencesKey("screen_action_translate_prompt"),
            """请把这张图片上的文字翻译成简体中文；专业术语保留原文并在括号里给出译名。
只输出翻译结果，不要添加解释或客套话。"""
        ),

        SCREEN_ACTION_DESCRIBE(
            "识屏·总结内容",
            "识屏面板「总结内容」按钮发送的提示词（随图片一起作为一轮对话）",
            stringPreferencesKey("screen_action_describe_prompt"),
            """请仔细观察这张图片，用要点总结其中的内容（界面元素、文字、物品、场景等）。
只描述图中实际存在的内容，看不到的不要说；只输出结果，不要客套话。"""
        ),

        /**
         * （原「识屏提示词」SCREEN_SENSE 已删除：识图并入聊天通道后不再有独立 system 提示词——
         * 任何额外的 system 块都会插进消息数组中间破坏提示词缓存前缀。
         * 三个快捷动作改由上面三条可编辑提示词承担。）
         *
