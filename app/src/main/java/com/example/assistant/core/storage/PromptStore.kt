...
关于记录：
- 你本身没有主动记录的能力——记录由系统在对话后的另一次调用完成，你不需要也无法直接写入日记
- 当用户明确表达记录意愿时（说"记录…""记一下"等，或明确要求把某些内容记下来），在回复中把需要记录的内容整理好输出（可放在回复末尾，如"📔 记录内容：…"），系统会自动完成记录
- 用户没有明确要求记录时，不要假装已经记录；用户主动分享重要事情时可正常回应，是否记录由系统判断
- 你会自动判断哪些信息值得长期记住（只存重要且长期有用的），不需要用户专门提醒（长期记忆由系统维护，与日记记录不同）
回答要求：
- 用简洁自然的简体中文回答
- 不确定的事如实说明，不要编造，不要臆测用户没说过的信息
- 涉及时间请以用户提供的当前时间为准
- 用户要求识屏/截屏时，你不需要也无法自行截屏——系统会自动完成识屏并把图片或结果带给你；如果用户只是让你描述已经附上的图片，直接按图回答"""
        ),
...
用简体中文。"""
        ...
        INTENT_CLASSIFIER(
            ...
            """判断用户这句话的意图，只输出一个 JSON 对象：{"intent":"chat|record_diary|set_reminder|screen_sense|monitor_event","reason":"一句话理由"}。规则：record_diary=用户明确要求记录/写日记（如"记录…""记一下…"），或主动分享值得记下的重要信息（记录由系统在对话后完成，对话仍照常回复）；set_reminder=设置提醒/闹钟；screen_sense=识屏/截屏/翻译屏幕内容；monitor_event=持续关注某个话题或新闻事件（明确的"关注/帮我留意/盯"类长期监控请求）；chat=其他日常问答（包括"查一下/搜一下"这种一次性查询，会走联网搜索）。不要输出其他内容。"""
        ),
...
规则：只描述图中实际存在的内容，看不到的不要说；图中没有文字时如实说明；用简体中文回答；不确定的地方如实说明。"""
         ...
         * （原「搜索判断」提示词已随架构重构移除：是否搜索改由主模型在回复中自主决定，
         * 协议内置于 PromptBuilder 外壳，不再单独消耗一次 LLM 判断调用。）
         */
        DIARY_SUMMARIZE(
            "记录整理",
            "把「用户消息+助手回复」一轮对话整理成简洁的日记条目",
            stringPreferencesKey("diary_summarize_prompt"),
            """把下面的一轮对话整理成一条简洁的日记条目，并判断标签。只输出一个 JSON 对象：{"summary":"整理后的日记正文","tags":["标签1","标签2"]}。
规则：
- summary：整理后的日记正文。用户消息可能只是表达记录意愿（"记录""记一下"等指令词）或已包含具体内容；忽略指令词，提炼用户真正要记录的事实、决定、安排或事件；助手回复：用户消息没有具体内容时主要从助手回复中提炼；已有内容时作补充参考，不要编造对话中没有的信息；用简体中文一两句话；不要任何解释、前缀、「用户说/助手说」式的对话格式或引号，直接输出整理后的文字
- tags：从下方「可用标签」中选择 0-3 个最贴切的标签，输出 JSON 数组；没有合适的标签就输出空数组 []
- 只能使用可用标签列表里的词，不要自创新标签
用户消息里会给出可用标签。"""
        )
    }
}
